...
class PDFLearningAssistant:
    ...
    def load_document(self, pdf_path: str) -> Dict[str, Any]:
        ...
        if Path(pdf_path).suffix.lower() != ".pdf":
            return {"success": False, "message": "目前仅支持加载 PDF 文件"}
        ...
        try:
            ...
            # HelloAgents 工具会把部分异常包装为字符串，不能仅依赖 try/except。
            if not isinstance(result, str):
                return {"success": False, "message": "RAG 工具返回了无法识别的结果"}
            if result.lstrip().startswith(("❌", "⚠️")):
    ...
    def add_note(self, content: str, concept: Optional[str] = None) -> str:
        ...
        Args:
            ...
            concept: 相关概念（可选）
        """
        memory_content = f"[概念：{concept}] {content}" if concept else content
        ...
        if isinstance(result, str) and not result.lstrip().startswith(("❌", "⚠️")):
...
def create_gradio_ui():
    ...
    def init_assistant(user_id: str) -> str:
        """初始化助手"""
        user_id = (user_id or "web_user").strip() or "web_user"
        assistant_state["assistant"] = None
        last_error = None
        ...
        for attempt in range(1, INIT_MAX_ATTEMPTS + 1):
            try:
                ...
                if not getattr(assistant.rag_tool, "initialized", False):
                    init_error = getattr(
                        assistant.rag_tool,
                        "init_error",
                        "RAG 工具初始化状态异常"
                ...
                return f"✅ 助手已初始化 (用户: {user_id}{retry_note})"
        ...
        if "UNEXPECTED_EOF_WHILE_READING" in error_message:
            return (
                "❌ 初始化失败：Qdrant 的 TLS 连接被代理或网络链路提前中断。\n"
                "请切换代理节点，或在代理软件中为 qdrant.io 配置可用线路后重试。"
            )

        return f"❌ 初始化失败（已重试 {INIT_MAX_ATTEMPTS} 次）：{error_message}"
    ...
    def add_note_ui(note_content: str, concept: str) -> str:
        ...
        result = assistant_state["assistant"].add_note(note_content, concept or None)
